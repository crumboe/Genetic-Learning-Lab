...
class GeneticAlgorithm:
    def __init__(self, population_size=None, generations=None, visualize_best=None,
                 kp_range=None, ki_range=None, kd_range=None,evaluate_pid = None):
        """
        Args:
        ...
        self.evaluate_pid = evaluate_pid
    ...
    def initialize_population(self):
        ...
        for _ in range(self.population_size):
            kp = self.kp_range[0] + np.random.uniform(0,1) * kp_size
            ki = self.ki_range[0] + np.random.uniform(0,1) * ki_size
            kd = self.kd_range[0] + np.random.uniform(0,1) * kd_size
    ...
    def evaluate_population(self, population, current_gen):
        ...
        for idx, ind in enumerate(population):
            ...
            fit = self.evaluate_pid(ind[0], ind[1], ind[2], render=render, ga_instance=self, current_gen=current_gen,episodes=EPISODES_PER_EVAL,current_individual=idx+1)
            fitness.append(fit)
 ...
 #render the best individual of the generation
            self.evaluate_pid(population[best_inx][0], population[best_inx][1], population[best_inx][2], render=True,
